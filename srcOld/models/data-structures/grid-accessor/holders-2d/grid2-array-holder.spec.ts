...
describe("GridHolderStatic", () => {
    describe("Test getters", () => {
        ...
        describe("Test get getAreaData", () => {
            it("With size 1", () => {
                expect((holder as any).getAreaInternally({ x: 0, y: 0 }, { x: 1, y: 1 }, "block")).to.deep.equal([{
                    item: "a",
                    coordinates: { x: 0, y: 0 },
                }]);
                expect((holder as any).getAreaInternally({ x: 1, y: 0 }, { x: 1, y: 1 }, "block")).to.deep.equal([{
                    item: "b",
                    coordinates: { x: 1, y: 0 },
                }]);
                expect((holder as any).getAreaInternally({ x: 0, y: 1 }, { x: 1, y: 1 }, "block")).to.deep.equal([{
                    item: "e",
                    coordinates: { x: 0, y: 1 },
                }]);
                expect((holder as any).getAreaInternally({ x: 3, y: 3 }, { x: 1, y: 1 }, "block")).to.deep.equal([{
                    item: "p",
                    coordinates: { x: 3, y: 3 },
                }]);
            });
