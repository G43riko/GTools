...
describe("filterItemsString", () => {
  ...
  it.only("filter nested properties", () => {
    const GROUP_A = {
    ...
    expect(filterItemsString(groups, "persons.*.age:<15")).toEqual([GROUP_A]);
    expect(filterItemsString(groups, "persons.*.age:<34")).toEqual([GROUP_A, GROUP_B]);
  });
