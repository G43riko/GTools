...
describe("Histogram.printHist", () => {
    ...
    it("returns object when flat=false with bars scaled to max and counts appended", () => {
        const out = hist.printHist(hist as any, 10, { fillEmpty: false, appendValue: true, flat: false, minOccurences: 1 }) as Record<string, string>;
    ...
    it("respects minOccurences filter", () => {
        const out = hist.printHist(hist as any, 10, { fillEmpty: false, appendValue: true, flat: false, minOccurences: 2 }) as Record<string, string>;
    ...
    it("returns lines when flat=true and omit counts when appendValue=false", () => {
        const lines = hist.printHist(hist as any, 10, { fillEmpty: false, appendValue: false, flat: true, minOccurences: 1 }) as string[];
