...
export class Voronoi {
     ...
     * @returns {Trinity<number>} The IDs of the points comprising the given triangle.
     */
    private pointsOfTriangle(t: number): Trinity<number> {
        return this.edgesOfTriangle(t).map((edge) => this.delaunay.triangles[edge]) as Trinity<number>;
    }
     ...
     */
    private trianglesAdjacentToTriangle(t: number): Trinity<number> {
        return this.edgesOfTriangle(t).map((edge) => {
            const opposite = this.delaunay.halfedges[edge];

            return this.triangleOfEdge(opposite);
        }) as Trinity<number>;
    }
     ...
     */
    private triangleCenter(t: number): ReadonlyPair<number> {
     ...
     * @param {number} t The index of the triangle
     * @returns {Trinity<number>} The edges of the triangle.
     */
    private edgesOfTriangle(t: number): ReadonlyTrinity<number> {
     ...
     */
    private circumcenter(a: Pair<number>, b: Pair<number>, c: Pair<number>): Pair<number> {
        const [ax, ay] = a;
        const [bx, by] = b;
        const [cx, cy] = c;
    ...
    public renderCellVertices(context: CanvasRenderingContext2D): void {
        this.cells.v.forEach((cellVertices, cellIndex) => {
