...
import { parseBooleanValue, parseDuration, parseSize } from "./parser-utils.ts";
import { DurationUnit } from "../../enums/src/duration-unit.ts";
...
describe("parserUtils", () => {
  ...
  describe("parseSize", () => {
    ...
    it("parses binary units (power-of-two)", () => {
      ...
      expect(
        [1125899906842, 1125899906843].includes(parseSize("0.001pib"))
      ).toBeTruthy();
    });
    ...
    it("throws on unknown units", () => {
      ...
      expect(() => parseSize("20megabytes")).toThrow(
        'Invalid string: "20megabytes"'
      );
    });

    it("throws on invalid input", () => {
      expect(() => parseSize("hello world")).toThrow(
  ...
  describe("parseDuration", () => {
    ...
    it("parses minutes", () => {
      expect(parseDuration("3m")).toBe(3 * 60_000);
    });
    ...
    it("parses combined durations", () => {
      const result = parseDuration("1h 30m 10s 500ms");
      expect(result).toBe(1 * 3_600_000 + 30 * 60_000 + 10 * 1000 + 500);
    });

    it("parses mixed formatting (spaces, uppercase)", () => {
      expect(parseDuration("1H 5M 20S")).toBe(
        1 * 3_600_000 + 5 * 60_000 + 20 * 1000
      );
    });

    it("ignores 'ms' when matching 'm' (no conflict)", () => {
      expect(parseDuration("1m 10ms")).toBe(60_000 + 10);
    });

    it("parses string with leading/trailing spaces", () => {
      expect(parseDuration("   2h 5m  ")).toBe(2 * 3_600_000 + 5 * 60_000);
    });
    ...
    it("converts to seconds", () => {
      expect(parseDuration("2000ms", DurationUnit.S)).toBe(2);
    });

    it("converts to minutes", () => {
      expect(parseDuration("120s", DurationUnit.M)).toBe(2);
    });

    it("converts to hours", () => {
      expect(parseDuration("7200s", DurationUnit.H)).toBe(2);
    });

    it("converts to days", () => {
      expect(parseDuration("48h", DurationUnit.D)).toBe(2);
    });
  });
  describe("parseSize", () => {
    it("parses bytes with no unit", () => {
    ...
    describe("parseSize - invalid inputs", () => {
      it("throws on gibberish", () => {
        expect(() => parseSize("ten megabytes")).toThrow(
          'Invalid string: "ten megabytes"'
        );
        expect(() => parseSize("")).toThrow('Invalid string: ""');
        // expect(() => parseSize("12..3mb")).toThrow('Invalid string: "12..3mb"');
        ...
        expect(() => parseSize("3kb extra")).toThrow(
          'Invalid string: "3kb extra"'
        );
      });
    });
  });
