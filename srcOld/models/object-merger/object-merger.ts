import { createFilledArray, deepEqual } from "../../utils";
...
): { same: [T, T][]; missingInA: T[]; missingInB: T[] };
function findArrayDiff<T>(
    arrA: T[],
    arrB: T[],
    comparator: (a: T, b: T) => number,
...
function findArrayDiff<T>(
    ...
    comparator: (a: T, b: T) => number,
    merger?: (a: T, b: T) => T,
): { same: (T | [T, T])[]; missingInA: T[]; missingInB: T[] } {
...
export class ObjectMerger {
    ...
    private static mergeArrayWithComparator<T>(
        valueA: T[],
        valueB: T[],
        config: ObjectMergeDefinitions<T>,
        parent?: string,
    ): ObjectMergerResult<T[]> {
        ...
        const arrayResult: ObjectMergerResult<unknown>["arrayResult"] = [
            ...
            ...nonMergedDiff.same.map(([valA, valB], index) =>
                ObjectMerger.mergeProperty(valA, valB, config, parent ? `${parent}.${index}` : String(index))
            ),
    ...
    public static mergeArray<T>(
        ...
        if (config.comparator) {
            if (config.merger) {
                return ObjectMerger.mergeArrayWithComparatorMerger(
                    valueA,
                    valueB,
                    config.comparator,
                    config.merger,
    ...
    public static mergeObject<T>(
        ...
        const uniqueKeys = Array.from(
            new Set([...Object.keys(objA ?? {}), ...Object.keys(objB ?? {})] as (string & keyof T)[]),
        );
        ...
        const objectResult = uniqueKeys.reduce((acc, key) => {
            ...
            const result = ObjectMerger.mergeProperty(
                objA[key],
                objB[key],
                propertyConfig,
        ...
        if (strA.trim() === strB.trim()) {
            if (typeof valueA === typeof valueB) {
                return {
                    ...
                    mergedResult: config.merger
                        ? config.merger(valueA, valueB)
                        : (config.keepIndent ? undefined : strA.trim() as any),
                };
            }
            ...
            return {
                ...
                mergedResult: config.merger
                    ? config.merger(valueA, valueB)
                    : (config.keepIndent || config.keepTypes ? undefined : strA.trim() as any),
            };
        }
