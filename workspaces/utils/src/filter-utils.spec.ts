...
describe("createFilter", () => {
    ...
    it.skip("ENUM operator fails when custom filter missing", () => {
        expect(() => createFilter("unknownFilter")).toThrow(
            "'unknownFilter' is not a custom filter",
        );
    });
...
describe("filterItemsString", () => {
    ...
    describe("filter nested properties", () => {
        const GROUP_A = {
        ...
        it("should test default operator", () => {
            expect(filterItemsString(groups, "persons.*.age:<:10")).toEqual([]);
            expect(filterItemsString(groups, "persons.*.age:<:15")).toEqual([]);
            expect(filterItemsString(groups, "persons.*.age:<:34")).toEqual([
                GROUP_A,
            ]);
            expect(filterItemsString(groups, "persons.*.age:<:50")).toEqual([
                GROUP_A,
                GROUP_B,
            ]);
        });

        it("should test EVERY operator", () => {
            expect(filterItemsString(groups, "persons.*.age:&<:10")).toEqual([]);
            expect(filterItemsString(groups, "persons.*.age:&<:15")).toEqual([]);
            expect(filterItemsString(groups, "persons.*.age:&<:34")).toEqual([
                GROUP_A,
    ...
    it.skip("works with custom filters", () => {
        ...
        expect(result).toEqual([{ id: 2, age: 20 }]);
    });
});
