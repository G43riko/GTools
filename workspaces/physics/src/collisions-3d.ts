...
import { pointLine3dDistance, pointPoint3dDistance } from "./distances-3d.ts";
import { intersection3dVectorSquare } from "./intersects-3d.ts";
...
/**
 * @param aX - x center of capsule
 * @param aY - y coordinate of bottom part of bottom sphere
 * @param aZ - y center of capsule
 * @param aRadius
 * @param aHeight - height from the bottom  of  bottom sphere to the top of the top sphere
 * @param bX
 * @param bY
 * @param bZ
 */
export function collision3dCapsulePoint(
    aX: number,
    aY: number,
    aZ: number,
    aRadius: number,
    ...
    if (bY < aY) { // is under capsule
        return false;
    }

    if (bY > aY + aHeight) { // is above capsule
        return false;
    }

    if (distance2dPointPoint(bX, bZ, aX, aZ) > aRadius) {
        return false;
    }

    // inside bottom sphere
    if (pointPoint3dDistance(aX, aY + aRadius, aZ, bX, bY, bZ) <= aRadius) {
        return true;
    }

    // inside top sphere
    if (pointPoint3dDistance(aX, aY + aHeight - aRadius, aZ, bX, bY, bZ) <= aRadius) {
        return true;
    }

    // check if is inside body cylinder
    return bY > aY + aRadius && bY < aY + aHeight - aRadius;
}
