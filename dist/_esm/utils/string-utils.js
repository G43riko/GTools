...
export function cutUsing(text, maxLength, suffix = "...", lengthIncludeSuffix = true) {
...
export function template(text, values, start = "{{", end = "}}") {
    ...
    return text.replace(new RegExp(`${updatedStart}(.+?)${updatedEnd}`, "g"), (math, key) => String(values[key]));
}
export function removeEmptyLines(content) {
    return content.replace(/^\s*$(?:\r\n?|\n)/gm, "");
}
export function between(text, key1, key2, trim = false) {
    const processResult = (result) => trim ? result.trim() : result;
...
export function occurrences(text, key, overlapping = false) {
