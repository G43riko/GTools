...
describe("ObjectMerger", () => {
    it("It should test basic types merging", () => {
        const options: ObjectMergeDefinitions<string> = {
            comparator: (a, b) => 0,
            merger    : (a, b) => typeof a === "string" ? a.trim() : a,
        };

        expect(ObjectMerger.mergeProperty("gabo", "gabo", options)).to.deep.equal({
            mergedResult: "gabo",
        ...
        expect(ObjectMerger.mergeProperty({}, {} as any, options)).to.deep.equal({
            mergedResult: {},
            matchType   : ObjectMergeMatchType.EQUALS,
            valueA      : {},
            valueB      : {},
        ...
        expect(ObjectMerger.mergeProperty(true, "true" as any, options)).to.deep.equal({
            mergedResult: true,
            matchType   : ObjectMergeMatchType.MISS_TYPE,
            valueA      : true,
            valueB      : "true",
        });
        expect(ObjectMerger.mergeProperty(false, "false" as any, options)).to.deep.equal({
            mergedResult: false,
            matchType   : ObjectMergeMatchType.MISS_TYPE,
            valueA      : false,
            valueB      : "false",
        });
        expect(ObjectMerger.mergeProperty("123", 123 as any, options)).to.deep.equal({
            mergedResult: "123",
            matchType   : ObjectMergeMatchType.MISS_TYPE,
            valueA      : "123",
            valueB      : 123,
